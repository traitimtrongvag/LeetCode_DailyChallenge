...
You are given a 
0-indexed 
circular string array `words` and a string `target`. A 
...
Return *the 
shortest distance needed to reach the string* `target`. If the string `target` does not exist in `words`, return `-1`.
