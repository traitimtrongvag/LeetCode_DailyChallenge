...
lowercase English letters. You are also given two 
0-indexed character arrays `original` and `changed`, and an integer array `cost`, where `cost[i]` represents the cost of changing the character `original[i]` to the character `changed[i]`.
...
Return *the 
minimum cost to convert the string *`source`* to the string *`target`* using 
