# Problem 4110: Count Stable Subarrays
...
no inversions, i.e., there is no pair of indices `i nums[j]`.

You are also given a 
2D integer array `queries` of length `q`, where each `queries[i] = [li, ri]` represents a query. For each query `[li, ri]`, compute the number of 
stable subarrays that lie entirely within the segment `nums[li..ri]`.
...
**Explanation:** - For `queries[0] = [0, 1]`, the subarray is `[nums[0], nums[1]] = [2, 2]`.
